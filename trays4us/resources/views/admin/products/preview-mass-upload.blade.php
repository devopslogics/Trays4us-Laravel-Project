               ...
               <div class="ftu-fail-img-path" >
                 <div class="ftu-path-title" >
                    <p>
                        ...
                        {{$fail_product->fetaure_image_orginal_name}}</p>
                  </div>
                   <?php /*
                  <div class="ftu-remove-path" >
                    ...
                    <div class="ftu-upload-data-box" >
                        <div class="row">
                         <div class="col-xl-5" >
                            <p class="ftu-title-status" ><input type="text" value="{{$fail_product->product_name}}" name="mu_product_name" data-sku="{{$fail_product->product_sku}}" data-mupid="{{$fail_product->id}}" class="mu_product_name"></p>
                            <h2 class="ftu-uploaded-product-title" >{{$fail_product->artist->display_name ? $fail_product->artist->display_name : $fail_product->artist->first_name.' '.$fail_product->artist->last_name}}</h2>
                            <div class="row" >
                              <div class="col-xl-6 tfu-left-uploaded-result" >
                                  ...
                                  <p>{{ $getProductTypeDetail->child->type_name ?? '' }}</p>
                                  <p>{{ $getProductTypeDetail->type_name ?? ''}}</p>

                                 <p class="tfu-sku-fail" >{{$fail_product->product_sku}} repeated SKU !</p>

                                  <p class="tfu-price-success">${{$fail_product->price}} / item</p>
                                  <p>MOQ: <span>{{$fail_product->minimum_order_quantity}}</span></p>
                              </div>
                              <div class="col-xl-6  tfu-right-uploaded-result " >
                                  <p>{{$fail_product->style->style_name}}</p>
                                  <p>{{$fail_product->country->country_name ?? ''}}</p>
                                  <p>{{$fail_product->state->state_name ?? ''}}</p>
                              </div>
                            </div>
                             @if($tags)
                                 <?php /*
                                 ...
                                 <div class="tfu-uploaded-tags-box tag_wrapper">
                                     ...
                                     <input type="hidden" name="tag_ids" data-sku="{{$fail_product->product_sku}}" class="tag-ids massu_tag_ids" value="{{$fail_product->tags}}">
                         ...
                         <div class="col-xl-7" >
                             @if($fail_product->images)
                                 <ul class="ftu-mass-image-gallery-result sortable">
                                     @foreach($fail_product->images as $gallery_image)

                                         @if($fail_product->feature_image == $gallery_image->image_name)
                                             <li class="ui-state-default selector ftu-img-handle feature_image" id="{{$gallery_image->id}}"><img src="{{ url('uploads/temp_products/small-'.$gallery_image->image_name) }}" ></li>
                                         @endif
                                     ...
                                     @foreach($fail_product->images as $gallery_image)
                                         @if($fail_product->feature_image != $gallery_image->image_name)
                                             <li class="ui-state-default selector ftu-img-handle" id="{{$gallery_image->id}}"><img src="{{ url('uploads/temp_products/small-'.$gallery_image->image_name) }}" ></li>
                                         @endif
                                     @endforeach
                                 </ul>
                             @endif
                            <p class="tfu-image-gallery-result-box" >{{$fail_product->product_description}}</p>
                         </div>
                    </div>
                </div>
            </div>
                ...
                @foreach($products as $product)
                 <div class="tfu-upload-row-result" >
                    ...
                    <div class="ftu-fail-img-path" >
                      <div class="ftu-path-title" >
                        <p><span><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15" fill="none">
                            ...
                            </svg></span>{{$product->fetaure_image_orginal_name}}</p>
                      </div>
                        <?php /*
                        <div class="ftu-remove-path" >
                            <img src="{{ asset('/assets/images/isolation_Mode.svg') }}"  />
                    ...
                    <div class="ftu-upload-data-box" >
                        ...
                        <div class="col-xl-5" >
                            <p class="ftu-title-status" ><input type="text" value="{{$product->product_name}}" name="mu_product_name" data-sku="{{$product->product_sku}}" data-mupid="{{$product->id}}" class="mu_product_name"></p>
                            <h2 class="ftu-uploaded-product-title" >{{$product->artist->display_name ? $product->artist->display_name : $product->artist->first_name.' '.$product->artist->last_name}}</h2>
                            <div class="row" >
                                <div class="col-xl-6 tfu-left-uploaded-result" >
                                ...
                                <p class="tfu-price-success"  >${{$product->price}} / item</p>
                                <p>MOQ: <span>{{$product->minimum_order_quantity}}</span></p>
                                </div>
                                <div class="col-xl-6  tfu-right-uploaded-result " >
                                <p>{{$product->style->style_name}}</p>
                                <p>{{$product->country->country_name ?? ''}}</p>
                                <p>{{$product->state->state_name ?? ''}}</p>
                                </div>
                            </div>

                                <div class="tfu-uploaded-tags-box tag_wrapper">

                                    <input type="hidden" name="tag_ids[{{$product->id}}]" data-sku="{{$product->product_sku}}" class="tag-ids massu_tag_ids" value="{{$product->tags}}">
                                    <div class="col-sm-10 tags-input">
                                        <div class="myTags" id="">
                                            <span class="data">  {!! $tags !!}</span>
                        ...
                        <div class="col-xl-7">
                            @if($product->images)
                                <ul class="ftu-mass-image-gallery-result sortable">
                                    @foreach($product->images as $gallery_image)

                                        @if($product->feature_image == $gallery_image->image_name)
                                            <li class="ui-state-default selector ftu-img-handle feature_image" id="{{$gallery_image->id}}"><img src="{{ url('uploads/temp_products/small-'.$gallery_image->image_name) }}" ></li>
                                        @endif
                            ...
                            @endif
                            <p class="tfu-image-gallery-result-box" >{{$product->product_description}}</p>
                        </div>
        ...
        $(document).ready(function() {
            ...
            $(".sortable").sortable({
                ...
                update: function (event, ui) {
                    ...
                    $.ajax({
                        method: "POST",
                        url:"{{route('sortable-mass-upload-images')}}",
                        data:{
                            "_token": "{{csrf_token()}}",
