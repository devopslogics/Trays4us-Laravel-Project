        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    ?>
                    <form action="{{ route('edit-product-submitted') }}" method="post" enctype="multipart/form-data" name="add_product" id="add_product_form" class="add_product dropzone">
                        {{ csrf_field() }}
                        ...
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="parent_product_type" id="parent_product_type">
                                        ...
                                        @foreach($parent_product_types as $parent_product_type)
                                            <option value="{{$parent_product_type->id}}" {{ $product_price->pt_parent_id == $parent_product_type->id ? 'selected' : ''}}>{{$parent_product_type->type_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label>Price</label>
                                    <input type="text" class="form-control" name="price" value="{{$product_price->price}}">
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="sub_product_type" id="sub_product_type">
                                        @foreach($sub_types as $sub_type)
                                            <option value="{{$sub_type->id}}" {{ $product_price->pt_sub_id == $sub_type->id ? 'selected' : ''}}>{{$sub_type->type_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="case_pack" id="case_pack" value="{{$moq_case_pack->case_pack ?? 0 }}" readonly>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="design_type" id="product_customizable">
                                        ...
                                        @foreach($customizables as $customizable)
                                            <option value="{{$customizable->id}}" {{ $product->product_customizable == $customizable->id ? 'selected' : ''}}>{{$customizable->customizable_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="minimum_order_quantity" id="minimum_order_quantity" value="{{$moq_case_pack->minimum_order_quantity ?? 0}}" readonly>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="badge" id="badge">
                                        ...
                                        @foreach($badges as $badge)
                                            <option value="{{$badge->id}}" data-custom="{{ $badge->custom }}" {{ $product->p_badge == $badge->id ? 'selected' : ''}}>{{$badge->badge}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="artist_id">
                                        ...
                                        @foreach($artists as $artist)
                                            ...
                                            @endphp
                                            <option value="{{$artist->id}}" {{ $product->artist_id == $artist->id ? 'selected' : ''}}>{{$artist_name}}</option>
                                        @endforeach
                                    </select>
                                </div>

                                <div class="form-group" id="customer_only" style="display: {{ $product->customer_id > 0 ? 'block' : 'none'}}">
                                    ...
                                    <input class="form-control" type="text" name="customer_name" id="customer_name" value="{{$product->customer->company ?? ''}} ( {{$product->customer_id ?? ''}} )">
                                    <input type="hidden" id="customer_id" name="customer_id" value="{{ (isset($product->customer_id) && $product->customer_id > 0) ? $product->customer_id : '' }}">
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group tag_wrapper">
                                    <label>Tags</label>
                                    <input type="hidden" name="tag_ids" id="tag_ids" class="tag-ids"  value="{{ collect($productTagIds)->join(', ') }}">
                                    <?php /*
                                    ...
                                    <div class="tags-input">
                                        <div class="myTags" id="">
                                            <span class="data">
                                                @if($product->tags)
                                                    @foreach($product->tags as $tag_row)
                                                        <span class="tag"><span class="text" _value="{{$tag_row->id}}">{{$tag_row->tag_name}}</span><span class="close">&times;</span></span>
                                                    @endforeach
                                                @endif
                                            </span>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="style_id">
                                        ...
                                        @foreach($themes as $theme)
                                            <option value="{{$theme->id}}" {{ $product->style_id == $theme->id ? 'selected' : ''}}>{{$theme->style_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="country_id" id="country_id">
                                        ...
                                        @foreach($countries as $country)
                                            <option value="{{$country->id}}" {{ $product->country_id == $country->id ? 'selected' : ''}}>{{$country->country_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="state_id" id="state_id">
                                        ...
                                        @foreach($states as $state)
                                            <option value="{{$state->id}}" {{ $state->id == $product->state_id ? 'selected' : ''}}>{{$state->state_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" name="description" id="custom_editor" rows="4" cols="50">{{$product->product_description}}</textarea>
                                </div>
                            </div>
                            <?php /*
                            <div class="col-md-6">
...
@push('scripts')
    ...
    <script>
        var gallery_images = {!! json_encode($product_gallery) !!}; // Remove the quotes around the JSON
        ...
        $(function() {
            var myDropzone = new Dropzone("div#dropzoneDragArea", {
                ...
                init: function() {
                    ...
                    var mockFile = { name: "{{ $image['name'] }}", size: 12345, url : "{{ $image['url'] }}" , id : "{{ $image['id'] }}" }; // You might need to adjust the size
                    myDropzone.emit("addedfile", mockFile);
                    myDropzone.emit("addFile", mockFile);
                    myDropzone.emit("thumbnail", mockFile, "{{ $image['url'] }}");

                   // myDropzone.addFile(mockFile);
                  //  myDropzone.createThumbnailFromUrl(file, "{{ $image['url'] }}");
