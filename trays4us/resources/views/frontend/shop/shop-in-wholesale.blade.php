            ...
            <div class="row">
                <div class="col-xl-12" >
                    <div class="alert alert-danger alejrt-block">
                        ...
                        <strong>Search "{{$_GET['search_by']}}" not found </strong>
                    </div>
                    <div class="tfu-alert tfu-alert-success" id="contact_email_wrapper">
                        ...
                        <form class="row tfu-email-design-form" action="">
                            <div class="col-8 col-sm-6  col-md-5  col-lg-3 p-1">
                              <input type="email" id="contact_email" value="{{Session::get('is_customer')->email ?? ''}}" class="form-control" id="tfu-email-design-input" placeholder="myemail@web.com">
                            </div>
            ...
            <div class="col-xl-12" >
              ...
              <div class="tfu-shop-filter-wrapper" >
                ...
                <div class="tfu-shop-filter-btn" >
                  <button class="tfu-btn-filter">Search Filter <span><img src="{{ asset('/assets/frontend-assets/svg/down_arrow.svg') }}" data-up="{{ asset('/assets/frontend-assets/svg/up_arrow.svg') }}" data-down="{{ asset('/assets/frontend-assets/svg/down_arrow.svg') }}"  alt="down-arrow.svg" /></span></button>
                  <a href="{{ route('frontend.products') }}" class="reset-btn-filter">Reset</a>
                                      ...
                                      @if ($product_types->isNotEmpty())
                                          @foreach($product_types as $product_type)
                                              <div class="form-check parent_ptype">
                                                  <input class="form-check-input tfu_parent_ptype" type="checkbox" name="parent_type[]" value="{{$product_type->id}}" id="pt_id_{{$product_type->id}}" {{(isset($_GET['parent_type']) && in_array($product_type->id,$_GET['parent_type'])) ? 'checked' : ''}}>
                                              ...
                                              @foreach($product_type->childTypes as $childType)
                                                  <div class="form-check child_ptype parent_{{$product_type->id}}">
                                                      <input class="form-check-input tfu_child_type" type="checkbox" name="child_type[]" data-parent-type="{{$product_type->id}}" value="{{$childType->id}}" id="pt_id_{{$childType->id}}" {{(isset($_GET['child_type']) && in_array($childType->id,$_GET['child_type'])) ? 'checked' : ''}}>
                                                ...
                                                */ ?>
                                                ...
                                                <select class="artists" id="aritst_id" data-placeholder="Stock Artists" multiple="true" name="artists[]" style="width:200px;">
                                                    @foreach($artists as $artists)
                                                        <option value="{{$artists->id}}" {{(isset($_GET['artists']) && in_array($artists->id,$_GET['artists'])) ? 'selected' : ''}}>{{$artists->display_name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                 ...
                                 <div class="tfu-shop-search-form">
                                    <div class="form-group">
                                      <span class="tfu-icon-input-filter-search">
                                        <img src="{{ asset('/assets/frontend-assets/svg/search-icon.svg')}}" alt="filter search">
                                      </span>
                                      <input type="text" class="form-control filter-search tfu_search_by" name="search_by" id="search_by" placeholder="Search..." value=" {{(isset($_GET['search_by']) && !empty($_GET['search_by'])) ? $_GET['search_by'] : ''}}">
                                    </div>
        ...
        <div class="row">
                 @if($products->isNotEmpty())
                   <div id="load_product_ajax">
                                ...
                                <div class="col-lg-3 col-sm-4 col-6" id="product_{{$product->pid}}">
                                    <div class="tfu-card-wrapper">
                                        ...
                                        <div class="tfu-card-header">
                                            ...
                                            <div class="tfu-card-img-overlay d-flex justify-content-end">
                                                @if(Session::has('is_customer') && !empty(Session::get('is_customer')))
                                                    ...
                                                    @if(isset($product->type) && ($product->type == 2))
                                                        <div class="tfu-dropdown">
                                                            ...
                                                            <ul class="tfu-dropdown-menu">
                                                                <li><a href="javascript:void(0)"  rel="nofollow" class="tfu_delete_custom_product" data-pid="{{ $product->pid }}">Delete</a></li>
                                                                <?php /*
                                                                <li>Copy</li>
                                                                <li>Edit</li>
                                                                <li>Make Public</li>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <a href="{{ route('product-detail',['slug' => $product->product_slug ]) }}">
                                                <h3 class="tfu-card-title" title="{!! $product->product_name !!}"> {!! Str::limit($product->product_name, 26, ' ...') !!}</h3>
                                            </a>
                    ...
                    <div class="col-xl-12" >
                        <button class="tfu-product-load-more" id="load_more_btn" style="display: {{ ($last_page > 1) ? 'block' : 'none' }}">See More...</button>
                    </div>
                 @else
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        var ENDPOINT = "{{ route('filter-products') }}";
        var page = 1;
        ...
        jQuery(document).ready(function(){
            ...
            $("#contact_email_btn").click(function(){
                ...
                $.ajax({
                    url: "{{ route('search-tag-designs-are-available') }}",
                    data: {
